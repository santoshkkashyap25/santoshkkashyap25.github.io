# Santosh Kumar Kashyap — Portfolio & Technical Blog

Personal portfolio and technical writing website for **Santosh Kumar Kashyap** ([@santoshkkashyap25](https://github.com/santoshkkashyap25)), AI / ML Engineer.

🌐 **Live Site**: [santoshkkashyap25.github.io](https://santoshkkashyap25.github.io/)

---

### Highlights
- **Projects**: Machine learning systems, RAG architectures, model quantization, and graph neural networks.
- **Interactive Terminal**: Embedded CLI shell emulator (`help`, `about`, `projects`, `blog`, `theme`, etc.).
- **Technical Writing**: Deep-dives and architectural notes on machine learning systems.
- **Design System**: Zero-build vanilla architecture with developer-focused monospace aesthetics and multi-theme support (`system`, `dark`, `matrix`, `dracula`, `light`).

---

### Links & Socials
- **Website**: [santoshkkashyap25.github.io](https://santoshkkashyap25.github.io/)
- **Substack**: [@skknotes](https://substack.com/@skknotes)
- **LinkedIn**: [santoshkkashyap25](https://www.linkedin.com/in/santoshkkashyap25/)
- **GitHub**: [@santoshkkashyap25](https://github.com/santoshkkashyap25)
